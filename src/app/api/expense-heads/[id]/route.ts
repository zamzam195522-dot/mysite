...
export async function GET(_request: Request, context: { params: Params }) {
  ...
  const result = await pool.query(`SELECT id, name, status FROM expense_heads WHERE id = $1 LIMIT 1`, [id]);
...
export async function PATCH(request: Request, context: { params: Params }) {
  ...
  let body: UpdateExpenseHeadRequest;
  try {
    body = (await request.json()) as UpdateExpenseHeadRequest;
  } catch {
...
export async function DELETE(_request: Request, context: { params: Params }) {
  ...
  // Safer to deactivate (expenses reference heads)
  const pool = getDbPool();
  const result = await pool.query(`UPDATE expense_heads SET status = 'INACTIVE' WHERE id = $1 RETURNING id`, [id]);
